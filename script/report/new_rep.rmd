...
count.path <- c("C:/Users/amedeo/Desktop/R_Projects/stool/data/ngs/merged_harmonized_converted.rds")
covar.path <- c("C:/Users/amedeo/Desktop/R_projects/stool/data/clinical/sensitivity/no_crc.RDS") 
...
4 - Alcohol: ~age + sex + library + alcohol. Age and sex used as adjustment variables. Alcohol was coded according to gr/day of alcohol intake and categorized in abstemious (0 - 0 gr/day), light drinker (0.1 – 18.5 gr/day), heavy drinker (>18.5 gr/day).
...
save(res, res_1, res_2, res_3, coldata, file = "C:/Users/amedeo/Desktop/R_Projects/stool/results/age/age.rda")
...
count.path <- c("C:/Users/amedeo/Desktop/R_projects/stool/data/ngs/merged_harmonized_converted.rds")
covar.path <- c("C:/Users/amedeo/Desktop/R_projects/stool/data/clinical/smoker.rds")
...
ggplot(data = countData.m, aes(x=study, y=log2(count))) + 
  ...
  facet_wrap( ~ miRNA, scales="free")+
  scale_fill_discrete(name = "Alcohol consumption", labels = c("Abstemious", "Light drinker [0.1-18.5 gr/day]", "Heavy drinker [>18.5 gr/day"))
...
```{r wine volcano, echo=FALSE, warning=FALSE}
...
```{r upset plot, echo=FALSE, warning=FALSE, fig.height=9, fig.width=8}
...
ht <- UpSet(m,
            set_order = c("Sex", "Wine", "Age", "Ncigs", "BMI", "Smoke", "Alcohol", "Physical"),
...
a <- intersect(sex,cigs)
b <- intersect(wine,age)
c <- intersect(sex,age)
d <- intersect(sex,wine)
e <- intersect(wine,bmi)
f <- intersect(cigs,age)
g <- intersect(cigs,smoke)
h <- intersect(cigs,bmi)
i <- intersect(cigs,wine)
l <- intersect(smoke,age)
m <- intersect(bmi,sex)
n <- intersect(bmi,smoke)
o <- intersect(smoke,wine)
p <- intersect(age,bmi)
q <- intersect(age,smoke)
r <- intersect(sex,alc)
s <- intersect(alcohol,wine)
t <- base::Reduce(intersect,list(wine,age,cigs))
u <- base::Reduce(intersect,list(sex,age,alc))
v <- base::Reduce(intersect,list(sex,wine,age))
z <- base::Reduce(intersect,list(cigs,age,smoke))
aa <- base::Reduce(intersect,list(age,smoke,wine))
bb <- base::Reduce(intersect,list(cigs,age,bmi))
cc <- base::Reduce(intersect,list(bmi,age,wine))
dd <- base::Reduce(intersect,list(wine,age,alc))
ee <- base::Reduce(intersect,list(cigs,bmi,sex))
ff <- base::Reduce(intersect,list(sex,wine,alc))
gg <- base::Reduce(intersect,list(wine,cigs,smoke))
hh <- base::Reduce(intersect,list(wine,cigs,bmi))
ii <- base::Reduce(intersect,list(sex,wine,age,alc))
ll <- base::Reduce(intersect,list(wine,age,cigs,smoke))
mm <- base::Reduce(intersect,list(wine,age,cigs,smoke))
# Below we listed the miRNAs shared between groups:
# - Sex and n° cigarettes: `r a`
```
