...
- Male vs female: `r n_res[[2]]` differentially expressed miRNAs.
...
xx <- grepl(paste0("(?=.*", "hsa|age_cat", ")", collapse=""), colnames(countData), perl=TRUE)
...
res_3 <- results(dds, contrast = c("smoke","fumatore", "non_fumatore"), alpha = 0.01)
res_3$miRNA <- rownames(res_3)
res_3 <- as_tibble(res_3) %>% 
  dplyr::relocate("miRNA", .before = "baseMean") %>% 
  dplyr::arrange(padj) %>% 
...
xx <- grepl(paste0("(?=.*", "hsa|smoke", ")", collapse=""), colnames(countData), perl=TRUE)
...
countData.aa <- countData[,1:aa]
countData.bb <- countData[,c(1,(aa+1):bb)]
countData.cc <- countData[,c(1,(bb+1):14)]
...
xx <- grepl(paste0("(?=.*", "hsa|curr_day_cat", ")", collapse=""), colnames(countData), perl=TRUE)
...
ggplot(data = countData.aa.m, aes(x=miRNA, y=log2(count))) + 
  ...
  facet_wrap( ~ miRNA, scales="free")+
  scale_fill_discrete(name = "N° cigarettes", labels = c("<16 cigs/day", ">16 cigs/day"))
...
xx <- grepl(paste0("(?=.*", "hsa|bmi_cat", ")", collapse=""), colnames(countData), perl=TRUE)
...
xx <- grepl(paste0("(?=.*", "hsa|alcool", ")", collapse=""), colnames(countData), perl=TRUE)
...
xx <- grepl(paste0("(?=.*", "hsa|phys_act", ")", collapse=""), colnames(countData), perl=TRUE)
...
ggplot(data = countData.m, aes(x=miRNA, y=log2(count))) + 
  ...
  facet_wrap( ~ miRNA, scales="free")+
  scale_fill_discrete(name = "Physical activity", labels = c("Active", "Inactive", "Moderately inactive"))
